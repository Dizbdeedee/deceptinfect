...
class InfectionLookSystem extends System {
	...
	override function init_server() {
		IterateEnt.onAdd([InfectionLookInfoAbility, PlayerComponent], [_, {player: lookerPlayer}]
			, function(lookerEnt) {
				IterateEnt.iterGet([InfectionComponent, PlayerComponent], [_, _], function(victimEnt) {
					if (lookerEnt == victimEnt)
						continue;
					createLinkEntities(lookerEnt, victimEnt, lookerPlayer);
					trace("Create linkEntities 1");
				});
		});
		IterateEnt.onAdd([InfectionComponent, PlayerComponent], [_, _], function(victimEnt) {
	...
	override function run_server() {
		...
		IterateEnt.iterGet([InfectionLookTime], [c_infLookTime = {looker: looker, victim: victim}]
			, function() {
				looker.hasExpr([PlayerComponent, InfectionLookInfoAbility],
					[{player: infLookPlayer}, {threshold: infAbilityThreshold}],
					function() {
						final hitEnt = infLookPlayer.GetEyeTrace()
							.Entity;
						victim.hasExpr([GEntityComponent, InfectionComponent]
							, [{entity: victimGEnt}, {infection: victimInfection}], function() {
								if (victimGEnt == hitEnt) {
									c_infLookTime.time += gameSystem.getGameManager()
										.diffTime;
								} else {
									c_infLookTime.time -= gameSystem.getGameManager()
										.diffTime;
								...
								if (c_infLookTime.time >= infAbilityThreshold) {
									IterateEnt.iterGet([InfectionLookData, ClientRepresentationTarget], [
										c_infLookData,
										c_crt = {
											target: crt
										}
									...
									IterateEnt.iterGet([InfectionLookData, ClientRepresentationTarget]
										, [c_infLookData, {target: crt}], function() {
											if (victim != crt)
												continue;
									});
								}
