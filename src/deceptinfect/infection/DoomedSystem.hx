...
class DoomedSystem extends System {
	...
	override function init_client() {
		componentManager.getAddSignal(ClassToID.idc(DoomClient))
			.handle((doom) -> {
	...
	override function run_server() {
		...
		IterateEnt.iterGet([DoomTarget, InfectionComponent], [c_dt, {rate: rate, infection: inf}]
			, function(victim) {
				if (victim.has_comp(Doomed))
					continue;
				...
				if (c_dt.idleTime > GameValues.DOOM_WAIT_FOR_IDLE) {
					trace("Now doomed :(");
					victim.add_component(new Doomed(Gmod.CurTime()
						+ MathLib.random(GameValues.DOOM_MIN, GameValues.DOOM_MAX)));
		...
		IterateEnt.iterGet([Doomed], [{doomTime: deth}], function(victim) {
			if (victim.has_comp(InfectedComponent))
				continue;
