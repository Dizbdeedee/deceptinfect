...
class ItemOwnerSystem extends System {
    ...
    public static final net_itemReveal = new gmod.net.NET_Server<"di_itemreveal",Net_ItemReveal>();
    #if client
	...
	var x = 0;
	deceptinfect.macros.ForEntities.forEnts(
	    switch (ent.get(ItemReveal)) {
	...
	// trace("runnin");
	deceptinfect.macros.ForEntities.forEnts(revealer(ent));
