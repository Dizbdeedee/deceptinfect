...
class Hud extends System {
	...
	function targetID() {
		var target:GEntCompat = Gmod.LocalPlayer()
			.GetEyeTrace()
			.Entity;
		...
		switch (target.has_id()) {
			case Some(_.get(InfectionLookData) => Comp(c_inf)):
				...
				SurfaceLib.SetFont("TargetID");
				SurfaceLib.DrawText(StringLib.format("Infection: %6.2f%%"
					, intpretInfection(c_inf.infection)));
			default:
		}
	...
	function batteryInfo() {
		IterateEnt.iterGet([BatteryInfo], [{charge: c}], function() {
			SurfaceLib.SetFont("DermaLarge");
			SurfaceLib.SetTextColor(255, 255, 255);
	...
	function infectionPoints() {
		IterateEnt.iterGet([InfectionPoints], [{points: p}], function() {
			SurfaceLib.SetFont("DermaLarge");
			SurfaceLib.SetTextColor(255, 255, 255);
