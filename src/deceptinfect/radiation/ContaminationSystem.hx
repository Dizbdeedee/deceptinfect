...
class ContaminationSystem extends System {
	...
	override function run_server() {
		...
		// 	, [c_rv, c_contamAccept, {pos: victimPos}], (vic) -> {
		// 		IterateEnt.iterGet([RadSource, ContaminationProducer, VirtualPosition]
		// 			, [c_rs, c_contamProduce, {pos: producePos}], (produce) -> {
		// 				if (c_rs.attatch == vic) {
		...
		// 									var rad = radSourceSystem.radSourceFromType(c_rs.type, vic
		// 										, c_rs.base);
