...
class Spread extends System {
	...
	function spread() {
		var removed = 0;
		IterateEnt.iterGet([GasDraw], [{ownedBy: owner}], function(ent) {
			...
			for (ep in endpoints) {
				for (ep2 in ep.GetAdjacentAreasAtSide(EAST)) {
					if (!markedMap.exists(ep2.GetID())) {
						final ent = componentManager.addEntity();
						ent.add_component(new GasDraw(ep.GetCorner(NORTH_EAST), ep.GetCorner(NORTH_WEST)
							, spreadEnt));
				...
				for (ep2 in ep.GetAdjacentAreasAtSide(SOUTH)) {
					if (!markedMap.exists(ep2.GetID())) {
						final ent = componentManager.addEntity();
						ent.add_component(new GasDraw(ep.GetCorner(SOUTH_EAST), ep.GetCorner(SOUTH_WEST)
							, spreadEnt));
						ent.add_component(new ReplicatedEntity());
						break;
					}
				...
				for (ep2 in ep.GetAdjacentAreasAtSide(WEST)) {
					if (!markedMap.exists(ep2.GetID())) {
						final ent = componentManager.addEntity();
						ent.add_component(new GasDraw(ep.GetCorner(NORTH_WEST), ep.GetCorner(SOUTH_WEST)
							, spreadEnt));
						ent.add_component(new ReplicatedEntity());
						break;
					}
