  ...
  if (_g[1] == 1) then 
    ...
    if (_g1[1] == 1) then 
      if (_g2[1] == 1) then 
        ...
        local dmgInfo = data.dmg;
        if (damageVal >= health) then 
          __haxe_Log.trace(Std.string(Std.string(Std.string("Did ") .. Std.string(damageVal)) .. Std.string(" points of damage to extra health: ")) .. Std.string(_g.extraHealth), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/ecswip/HiddenHealthSystem.hx",lineNumber=65,className="deceptinfect.ecswip.HiddenHealthSystem",methodName="hiddenHealthDamage"}));
          _g.extraHealth = _g.extraHealth - damageVal;
          if (_g.extraHealth > 0) then 
            dmgInfo:SetDamage(health - 1);
          else
          ...
          __haxe_Log.trace(Std.string(Std.string(Std.string("Adding ") .. Std.string(damageVal)) .. Std.string(" to ")) .. Std.string(damageVal * (1 - _g2.damagepenalty)), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/ecswip/HiddenHealthSystem.hx",lineNumber=73,className="deceptinfect.ecswip.HiddenHealthSystem",methodName="hiddenHealthDamage"}));
          _g.extraHealth = _g.extraHealth + (damageVal * (1 - _g2.damagepenalty));
        end;
      end;
    end;
...
__deceptinfect_radiation_RadiationSystem.prototype.run_server = function(self) 
  local this1 = __deceptinfect_ecswip_ComponentManager.entities;
  local acceptEnt_min = 0;
  while (acceptEnt_min < this1) do 
    acceptEnt_min = acceptEnt_min + 1;
    local acceptEnt = acceptEnt_min - 1;
    local _g = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_radiation_RadiationAccepter)[acceptEnt];
    local _g1 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_infection_RateComponent)[acceptEnt];
    local _g2 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_ecswip_VirtualPosition)[acceptEnt];
    local _g3 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_radiation_RadVictim)[acceptEnt];
    if (_g[1] == 1) then 
      local _g = _g[2];
      if (_g1[1] == 1) then 
        local _g1 = _g1[2];
        if (_g2[1] == 1) then 
          ...
          if (_g3[1] == 1) then 
            _g.radiation.h = ({});
            local this1 = __deceptinfect_ecswip_ComponentManager.entities;
            local produceEnt_min = 0;
            while (produceEnt_min < this1) do 
              produceEnt_min = produceEnt_min + 1;
              local produceEnt = produceEnt_min - 1;
              local _g1 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_radiation_RadiationProducer)[produceEnt];
              local _g3 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_ecswip_VirtualPosition)[produceEnt];
              local _g4 = __deceptinfect_ecswip_ComponentManager.lazyInit(__deceptinfect_radiation_RadSource)[produceEnt];
              if (_g1[1] == 1) then 
                if (_g3[1] == 1) then 
                  if (_g4[1] == 1) then 
