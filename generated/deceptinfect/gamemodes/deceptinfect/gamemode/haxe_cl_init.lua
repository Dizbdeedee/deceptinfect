...
NETMESSAGE_deceptinfect_Darken_net_darken.super = function(self,recvs) 
  ...
  _G.timer.Simple(0, function() 
    ...
    _gthis.init = true;
    _G.net.Receive("deceptinfect_Darken_net_darken", _hx_bind(_gthis,_gthis.receive));
  end);
  self.signal = self.signalTrigger;
  local _g = 0;
  while (_g < recvs.length) do 
    local recv = recvs[_g];
    _g = _g + 1;
    self.signal:listen(recv);
  end;
end
...
NETMESSAGE_deceptinfect_Darken_net_darken.prototype.receive = function(self) 
  local bytString = _G.net.ReadData(_G.net.BytesLeft());
  local bytes = __haxe_io_Bytes.ofString(bytString);
  local result = self.seralizer:unserialize(bytes, __deceptinfect_NDarken);
  self.signalTrigger.handlers:invoke(result);
end
...
NETMESSAGE_deceptinfect_Spread_netSpread.super = function(self,recvs) 
  ...
  _G.timer.Simple(0, function() 
    _gthis.seralizer = __hxbit_Serializer.new();
    _gthis.init = true;
    _G.net.Receive("deceptinfect_Spread_netSpread", _hx_bind(_gthis,_gthis.receive));
  end);
  self.signal = self.signalTrigger;
  local _g = 0;
  while (_g < recvs.length) do 
...
NETMESSAGE_deceptinfect_Spread_netSpread.prototype.receive = function(self) 
  local bytString = _G.net.ReadData(_G.net.BytesLeft());
  local bytes = __haxe_io_Bytes.ofString(bytString);
  local result = self.seralizer:unserialize(bytes, __deceptinfect_NetSpread);
  self.signalTrigger.handlers:invoke(result);
end
...
NETMESSAGE_deceptinfect_game_ClientTranslateSystem_net_updateServerEnt.super = function(self,recvs) 
  ...
  _G.timer.Simple(0, function() 
    _gthis.seralizer = __hxbit_Serializer.new();
    _gthis.init = true;
    _G.net.Receive("deceptinfect_game_ClientTranslateSystem_net_updateServerEnt", _hx_bind(_gthis,_gthis.receive));
  end);
  self.signal = self.signalTrigger;
  local _g = 0;
  while (_g < recvs.length) do 
...
NETMESSAGE_deceptinfect_game_ClientTranslateSystem_net_updateServerEnt.prototype.receive = function(self) 
  local bytString = _G.net.ReadData(_G.net.BytesLeft());
  local bytes = __haxe_io_Bytes.ofString(bytString);
  local result = self.seralizer:unserialize(bytes, __deceptinfect_game_Net_UpdateServerEnt);
  self.signalTrigger.handlers:invoke(result);
end
...
__hxbit_Serializer.prototype.unserialize = function(self,data,c,startPos) 
  if (startPos == nil) then 
    startPos = 0;
  end;
  self.refs = __haxe_ds_IntMap.new();
  self.knownStructs = _hx_tab_array({}, 0);
  self:setInput(data, startPos);
...
__deceptinfect_NDarken.prototype.unserializeInit = function(self) 
end
__deceptinfect_NDarken.prototype.unserialize = function(self,__ctx) 
  local pos = (function() 
  local _hx_obj = __ctx;
  local _hx_fld = 'inPos';
  local _ = _hx_obj[_hx_fld];
  ...
  self.darkenToggle = __ctx.input.b[pos] ~= 0;
end
...
__deceptinfect_NetSpread.prototype.unserializeInit = function(self) 
  self.endpoints = _hx_tab_array({}, 0);
end
__deceptinfect_NetSpread.prototype.unserialize = function(self,__ctx) 
  local e0;
  local pos = (function() 
  local _hx_obj = __ctx;
  local _hx_fld = 'inPos';
  ...
  if (len == 0) then 
    ...
    while (_g < _g1) do 
      ...
      if (fbits == 0) then 
        ...
        e0 = _hx_o({__fields__={northeast=true,southwest=true},northeast=northeast,southwest=southwest});
      end;
      a[i] = e0;
    end;
    tmp = a;
...
__gmod_helpers_net__NVector_NVector_Impl_.to = function(this1) 
  do return _G.Vector(this1[0], this1[1], this1[2]) end;
end
...
__deceptinfect_Spread.listen = function(ns) 
  __haxe_Log.trace(ns, _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/Spread.hx",lineNumber=151,className="deceptinfect.Spread",methodName="listen"}));
  local _g = 0;
  local _g1 = ns.endpoints;
  while (_g < _g1.length) do 
    local nvec = _g1[_g];
    _g = _g + 1;
    local northeast = __gmod_helpers_net__NVector_NVector_Impl_.to(nvec.northeast);
    local southwest = __gmod_helpers_net__NVector_NVector_Impl_.to(nvec.southwest);
    __deceptinfect_Spread.clendpoints:push(_hx_o({__fields__={northeast=true,southwest=true},northeast=northeast,southwest=southwest}));
    local this1 = northeast - southwest;
    local this1 = this1 / 2;
    local origin = this1 + southwest;
    _G.debugoverlay.Box(origin, southwest - origin, northeast - origin, 60, _G.Color(255, 0, 0));
  end;
end
...
__deceptinfect_Spread.prototype.run_client = function(self) 
  if (_G.CurTime() < self.nextGas) then 
    do return end;
  end;
  self.nextGas = _G.CurTime() + _G.math.Rand(0.3, 0.5);
  local _g = 0;
  local _g1 = __deceptinfect_Spread.clendpoints;
  while (_g < _g1.length) do 
    local point = _g1[_g];
    _g = _g + 1;
    local this1 = point.northeast;
    local otherVec = point.southwest;
    local this1 = this1 - otherVec;
    local this1 = this1 / 2;
    local otherVec = point.southwest;
    local this1 = this1 + otherVec;
    local otherVec = _G.Vector(0, -10, 0);
    local origin = this1 + otherVec;
    local vecRan = _G.VectorRand();
    vecRan:Normalize();
    local particledata = self.particleTable[0];
    local other = _G.math.Rand(20, 40);
    vecRan = vecRan * other;
    vecRan[3] = _G.math.Rand(10, 60);
    local emitter = _G.ParticleEmitter(origin);
    emitter:SetNearClip(5, 10);
    local radiusmul = 2.35294117647058831;
    local particle = emitter:Add(particledata.particle, origin + vecRan);
    particle:SetVelocity(_G.Vector((_G.math.Rand(-particledata.randXY, particledata.randXY) * radiusmul) * 2, (_G.math.Rand(-particledata.randXY, particledata.randXY) * radiusmul) * 2, _G.math.Rand(particledata.randZMin, particledata.randZMax) * radiusmul));
    ...
    particle:SetDieTime(_G.math.Rand(particledata.lifeTimeMin, particledata.lifeTimeMax));
    ...
    particle:SetRollDelta(_G.math.Rand(-particledata.rotRate, particledata.rotRate));
    emitter:Finish();
    emitter = nil;
    _G.collectgarbage("step", 64);
  end;
end
...
__deceptinfect_client_GeigerSystem.prototype.run_client = function(self) 
  ...
  while (_g < __lowest) do 
    ...
    if (__compArr[39][1][ent] ~= nil) then 
      local this1 = __compArr[39];
      local _g = this1[3][this1[1][ent]].geiger;
      __haxe_Log.trace("Running", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/client/GeigerSystem.hx",lineNumber=28,className="deceptinfect.client.GeigerSystem",methodName="run_client"}));
      self.geiger = _g;
    end;
  end;
  self:geigerThink();
end
__deceptinfect_client_GeigerSystem.prototype.geigerThink = function(self) 
  if ((_G.CurTime() > self.playTime) and (self.geiger > 0.016)) then 
    local x = self.geiger;
    ...
    local exponent = (0.5 * _G.math.pow(self.geiger, -0.5)) - 0.5;
    local random = math.random(0.5,1.1);
    local maxtime = 3;
    self.playTime = _G.math.min((_G.CurTime() + mintime) + (exponent * random), _G.CurTime() + maxtime);
  end;
end
...
__deceptinfect_client_Hud.targetID = function() 
  ...
  local player = target;
  _G.surface.SetTextPos(__deceptinfect_client_Hud.CSS(__deceptinfect_client__Hud_Axis.X, 900), __deceptinfect_client_Hud.CSS(__deceptinfect_client__Hud_Axis.Y, 540));
  ...
  if (_g[1] == 0) then 
    ...
    if (_hx_tmp[1] == 1) then 
      _G.surface.SetTextPos(__deceptinfect_client_Hud.CSS(__deceptinfect_client__Hud_Axis.X, 900), __deceptinfect_client_Hud.CSS(__deceptinfect_client__Hud_Axis.Y, 500));
      _G.surface.SetFont("TargetID");
...
__deceptinfect_ecswip_GrabSystem.updateSig = function(data) 
  if (__deceptinfect_ecswip_GrabSystem.activeDraws.h[data.index] == nil) then 
    ...
    __deceptinfect_ecswip_ComponentManager.addComponent(4, __deceptinfect_infection_components_GrabDraw.new(data.ent, data.ent2), draw);
...
__deceptinfect_game_RagdollSystem.prototype.recvraginfo = function(self,x) 
  ...
  if (_g[1] == 0) then 
    ...
    c_raginfo = (function() 
      ...
      _hx_2 = __deceptinfect_ecswip_ComponentManager.addComponent(12, __deceptinfect_game_components_RagInfo.new(), _g); else 
      _hx_2 = result:slice(2)[0]; end
      return _hx_2
    end )();
  else
    if (_G.IsValid(x.rag)) then 
      ...
      __deceptinfect_ecswip_ComponentManager.addComponent(12, c_rag, rag.id);
      c_raginfo = c_rag;
    else
      do return end;
    end;
...
__deceptinfect_game_RagdollSystem.prototype.recvstatinfo = function(self,x) 
  ...
  if (c_statinfo1) == 0 then 
    ...
    c_statinfo = (function() 
      local _hx_1
      if (result == __deceptinfect_ecswip_ComponentState.NONE) then 
      _hx_1 = __deceptinfect_ecswip_ComponentManager.addComponent(13, __deceptinfect_game_components_StatInfo.new(), _g); else 
  ...
  elseif (c_statinfo1) == 1 then 
    ...
    __deceptinfect_ecswip_ComponentManager.addComponent(13, c_stat, stat.id);
    c_statinfo = c_stat;
  ...
  if (_g[1] == 0) then 
    ...
    if (_g1[1] == 1) then 
      if (_g2[1] == 1) then 
        ...
        __haxe_Log.trace("THing changed", _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/game/RagdollSystem.hx",lineNumber=114,className="deceptinfect.game.RagdollSystem",methodName="pvsChange"}));
...
__deceptinfect_game_RagdollSystem.prototype.run_client = function(self) 
  ...
  while (rag_nextV ~= nil) do 
    ...
    if (((mdl ~= nil) and (mdl ~= "")) and (rag.showwep == nil)) then 
      ...
      __deceptinfect_ecswip_ComponentManager.addComponent(66, c_stat, _rag.id);
      __deceptinfect_ecswip_ComponentManager.addComponent(63, __deceptinfect_game_components_KeepRestart.new(), _rag.id);
      __haxe_Log.trace(mdl, _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/game/RagdollSystem.hx",lineNumber=171,className="deceptinfect.game.RagdollSystem",methodName="run_client"}));
      rag.showwep = true;
      ...
      c_wep.parent = rag;
      __deceptinfect_ecswip_ComponentManager.addComponent(67, c_wep, ent.id);
      __deceptinfect_ecswip_ComponentManager.addComponent(63, __deceptinfect_game_components_KeepRestart.new(), ent.id);
      ent:SetupBones();
...
__deceptinfect_infection_systems_InfectionLookSystem.prototype.recvInfectionInfo = function(self,x) 
  ...
  local c_inf = (function() 
    local _hx_1
    if (result == __deceptinfect_ecswip_ComponentState.NONE) then 
    _hx_1 = __deceptinfect_ecswip_ComponentManager.addComponent(37, __deceptinfect_infection_InfectionComponent.new(), target.id); else 
    _hx_1 = result:slice(2)[0]; end
    return _hx_1
  end )();
...
__deceptinfect_game_ItemOwnerSystem.prototype.init_client = function(self) 
  _G.hook.Add("PreDrawHalos", "di_drawitemhalos", _hx_bind(self,self.drawHalos));
end
__deceptinfect_game_ItemOwnerSystem.prototype.drawHalos = function(self) 
end
...
__deceptinfect_game_ClientTranslateSystem.componentsC = function() 
  local comp = __haxe_ds_IntMap.new();
  do return comp end;
end
__deceptinfect_game_ClientTranslateSystem.updateEnt = function(x) 
  local _g = 0;
  local _g1 = x.replEntities;
  while (_g < _g1.length) do 
    local ent = _g1[_g];
    _g = _g + 1;
    local ret = __deceptinfect_game_ClientTranslateSystem.serverIDToClientID.h[ent.serverID];
    if (ret == __haxe_ds_IntMap.tnull) then 
      ret = nil;
    end;
    local clientID = ret;
    if (clientID ~= nil) then 
      ...
      while (_g < _g1.length) do 
        local comp = _g1[_g];
        _g = _g + 1;
        local ret = __deceptinfect_game_ClientTranslateSystem.components.h[comp.__uid];
        if (ret == __haxe_ds_IntMap.tnull) then 
          ret = nil;
        end;
        local other = ret;
        if (__deceptinfect_game_ClientTranslateSystem.components.h[comp.__uid] == nil) then 
          local key = comp.__uid;
          local _this = __deceptinfect_game_ClientTranslateSystem.components;
          local value = comp;
          if (value == nil) then 
            _this.h[key] = __haxe_ds_IntMap.tnull;
          else
          ...
          __deceptinfect_ecswip_ComponentManager.addComponent(comp:getCompID(), comp, clientID);
        end;
        local _g = 0;
        local _g1 = Reflect.fields(comp);
        while (_g < _g1.length) do 
          local field = _g1[_g];
          _g = _g + 1;
          other[field] = Reflect.field(comp, field);
        end;
        __haxe_Log.trace(Std.string(Std.string(Std.string("") .. Std.string(Std.string(comp))) .. Std.string(" ")) .. Std.string(Std.string(other)), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/deceptinfect/game/ClientTranslateSystem.hx",lineNumber=81,className="deceptinfect.game.ClientTranslateSystem",methodName="updateEnt"}));
      end;
    else
      local id = nil;
      local _g = 0;
      ...
      while (_g < _g1.length) do 
        local comp = _g1[_g];
        _g = _g + 1;
        local compClass = Type.getClass(comp);
        if ((compClass == __deceptinfect_ecswip_PlayerComponent) or (compClass == __deceptinfect_ecswip_GEntityComponent)) then 
          local __compArr = __deceptinfect_ecswip_ComponentManager.components_3;
          local __n = 0;
          local __lowest = __compArr[61][4];
          local __lowest_comp = 61;
          local __lowestInt = __compArr[__lowest_comp][2];
          local _g = 1;
          local _g1 = __lowest;
          while (_g < _g1) do 
            ...
            local __int_id = _g - 1;
            local ent = __lowestInt[__int_id];
            if (__compArr[61][1][ent] ~= nil) then 
              local this1 = __compArr[61];
              local _g = this1[3][this1[1][ent]];
              local _g1 = _g.__uid;
              local c_currentEnt = _g;
              local ent = _g.entity;
              if (comp.entity == ent) then 
                id = __deceptinfect_ecswip_ComponentManager.lookupEntity.h[c_currentEnt];
              end;
            end;
          end;
          local __compArr = __deceptinfect_ecswip_ComponentManager.components_3;
          local __n = 0;
          local __lowest = __compArr[60][4];
          local __lowest_comp = 60;
          local __lowestInt = __compArr[__lowest_comp][2];
          local _g = 1;
          local _g1 = __lowest;
          while (_g < _g1) do 
            _g = _g + 1;
            local __int_id = _g - 1;
            local plyEnt = __lowestInt[__int_id];
            if (__compArr[60][1][plyEnt] ~= nil) then 
              local this1 = __compArr[60];
              local _g = this1[3][this1[1][plyEnt]];
              local _g1 = _g.__uid;
              local c_currentPlayer = _g;
              local ply = _g.player;
              if (comp.player == ply) then 
                id = __deceptinfect_ecswip_ComponentManager.lookupEntity.h[c_currentPlayer];
              end;
            end;
          end;
        end;
      ...
      if (id == nil) then 
        id = __deceptinfect_ecswip_ComponentManager.addEntity();
      end;
      local _g = 0;
      local _g1 = ent.comp;
      while (_g < _g1.length) do 
        ...
        __deceptinfect_ecswip_ComponentManager.addComponent(comp:getCompID(), comp, id);
      end;
      local key = ent.serverID;
      local _this = __deceptinfect_game_ClientTranslateSystem.serverIDToClientID;
      if (id == nil) then 
        _this.h[key] = __haxe_ds_IntMap.tnull;
      else
        _this.h[key] = id;
...
__deceptinfect_game_Net_ReplicatedEntity.prototype.unserializeInit = function(self) 
  self.comp = _hx_tab_array({}, 0);
end
__deceptinfect_game_Net_ReplicatedEntity.prototype.unserialize = function(self,__ctx) 
  local e0;
  local pos = (function() 
  local _hx_obj = __ctx;
  local _hx_fld = 'inPos';
  ...
  if (len == 0) then 
    ...
    while (_g < _g1) do 
      _g = _g + 1;
      local i = _g - 1;
      e0 = __ctx:getRef(__deceptinfect_ecswip_ReplicatedComponent, __deceptinfect_ecswip_ReplicatedComponent.__clid);
      a[i] = e0;
    end;
    tmp = a;
  end;
...
__deceptinfect_game_Net_UpdateServerEnt.prototype.unserializeInit = function(self) 
end
__deceptinfect_game_Net_UpdateServerEnt.prototype.unserialize = function(self,__ctx) 
  local e0;
  local pos = (function() 
  local _hx_obj = __ctx;
  local _hx_fld = 'inPos';
  ...
  if (len == 0) then 
    ...
    while (_g < _g1) do 
      _g = _g + 1;
      local i = _g - 1;
      e0 = __ctx:getRef(__deceptinfect_game_Net_ReplicatedEntity, __deceptinfect_game_Net_ReplicatedEntity.__clid);
      a[i] = e0;
    end;
    tmp = a;
  end;
  self.replEntities = tmp;
end
...
__deceptinfect_game_components_ClientWeaponStatue.prototype.getCompID = function(self) 
  do return 67 end
end
...
__hxbit_Convert.getDefault = function(t) 
  ...
  while (true) do repeat 
    ...
    elseif (tmp) == 17 then 
      do return 0 end;
    elseif (tmp) == 3 or (tmp) == 4 or (tmp) == 14 or (tmp) == 15 or (tmp) == 18 then 
...
__hxbit_enumSer_Hxbit_PropTypeDesc.doUnserialize = function(ctx) 
  ...
  elseif (b1) == 12 then 
    ...
    local __e = __hxbit_enumSer_Hxbit_PropTypeDesc.doUnserialize(ctx);
    _k = __e;
    do return __hxbit_PropTypeDesc.PAbs(_k) end;
  elseif (b1) == 13 then 
    local _k;
    local __e = __hxbit_enumSer_Hxbit_PropTypeDesc.doUnserialize(ctx);
    _k = __e;
...
local _hx_static_init = function()
  ...
  __deceptinfect_ecswip_ComponentManager.initComponent(67, "ClientWeaponStatue");
  ...
  __deceptinfect_ecswip_ComponentManager.initComponent(63, "KeepRestart");
  ...
  __deceptinfect_ecswip_ComponentManager.initComponent(62, "Spawned");
  __deceptinfect_ecswip_ComponentManager.initComponent(13, "StatInfo");
  __deceptinfect_ecswip_ComponentManager.initComponent(66, "Statue");
  ...
  __deceptinfect_Spread.netSpread = NETMESSAGE_deceptinfect_Spread_netSpread.new(_hx_tab_array({[0]=__deceptinfect_Spread.listen}, 1));
  ...
  __deceptinfect_game_ClientTranslateSystem.net_updateServerEnt = NETMESSAGE_deceptinfect_game_ClientTranslateSystem_net_updateServerEnt.new(_hx_tab_array({[0]=__deceptinfect_game_ClientTranslateSystem.updateEnt}, 1));
  
  __deceptinfect_game_ClientTranslateSystem.serverIDToClientID = __haxe_ds_IntMap.new();
  
  __deceptinfect_game_ClientTranslateSystem.components = __deceptinfect_game_ClientTranslateSystem.componentsC();
  ...
  __deceptinfect_game_components_ClientWeaponStatue.compID = 67;
  ...
  __deceptinfect_game_components_KeepRestart.compID = 63;
  ...
  __deceptinfect_game_components_Spawned.compID = 62;
  
  __deceptinfect_game_components_StatInfo.compID = 13;
  
  __deceptinfect_game_components_Statue.compID = 66;
